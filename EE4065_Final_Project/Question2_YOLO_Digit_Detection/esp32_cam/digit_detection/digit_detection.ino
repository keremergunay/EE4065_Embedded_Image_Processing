...
bool initCamera() {
    ...
    // GC2145 için ayarlar (JPEG desteklenmiyor!)
...
void preprocessImage(camera_fb_t* fb, int8_t* data) {
    ...
    for (int y = 0; y < INPUT_HEIGHT; y++) {
        for (int x = 0; x < INPUT_WIDTH; x++) {
            ...
            // 4 komşu piksel
            uint16_t p00 = img[y0 * src_w + x0];
            uint16_t p01 = img[y0 * src_w + x1];
            uint16_t p10 = img[y1 * src_w + x0];
            uint16_t p11 = img[y1 * src_w + x1];
            
            // Her piksel için grayscale hesapla
            auto toGray = [](uint16_t p) -> float {
            ...
            float g00 = toGray(p00);
            float g01 = toGray(p01);
            float g10 = toGray(p10);
            float g11 = toGray(p11);
...
"for(var i=0;i<lastDetections.length;i++){"
"var d=lastDetections[i];"
"var px=Math.round(d.x*320),py=Math.round(d.y*240),pw=Math.round(d.w*320),ph=Math.round(d.h*240);"
...
// DEBUG: Preprocessed 96x96 görüntüyü göster
...
static esp_err_t debug_input_handler(httpd_req_t *req) {
    ...
    // Preprocess yap ve debug buffer'a kaydet
    uint16_t* img = (uint16_t*)fb->buf;
    int src_w = fb->width;
    int src_h = fb->height;
    ...
    for (int y = 0; y < 96; y++) {
        for (int x = 0; x < 96; x++) {
            ...
            uint16_t p = img[src_y * src_w + src_x];
            uint8_t r = ((p >> 11) & 0x1F) << 3;
            uint8_t g = ((p >> 5) & 0x3F) << 2;
            uint8_t b = (p & 0x1F) << 3;
            uint8_t gray = (uint8_t)(0.299f * r + 0.587f * g + 0.114f * b);
            debug_preprocessed[y * 96 + x] = gray;
        }
    }
    esp_camera_fb_return(fb);
    
    // Grayscale'i RGB565'e çevir ve JPEG'e dönüştür
    uint16_t* rgb565 = (uint16_t*)malloc(96 * 96 * 2);
    if (!rgb565) {
        httpd_resp_send_500(req);
        return ESP_FAIL;
    }
    ...
    for (int i = 0; i < 96*96; i++) {
        uint8_t g = debug_preprocessed[i];
        rgb565[i] = ((g >> 3) << 11) | ((g >> 2) << 5) | (g >> 3);
    }
    
    size_t jpg_len = 0;
    uint8_t *jpg_buf = NULL;
    
    // frame2jpg kullanamayız, elle JPEG yapalım - veya raw gönder
    // Basit çözüm: Raw grayscale gönder, tarayıcıda görüntüle
    httpd_resp_set_type(req, "application/octet-stream");
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    httpd_resp_set_hdr(req, "Content-Disposition", "inline; filename=\"debug_96x96.raw\"");
    esp_err_t res = httpd_resp_send(req, (const char*)debug_preprocessed, 96*96);
